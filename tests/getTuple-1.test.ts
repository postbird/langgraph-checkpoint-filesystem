...
const TEST_ROOT_FOLDER = path.resolve(__dirname, './test-checkpoint-file-store-getTuple-1');

const FILE_SAVER = new FileSystemSaver({
  rootFolder: TEST_ROOT_FOLDER,
});

describe('getTuple-1: with checkpoint_id input', async () => {
  afterAll(async () => {
  ...
  const tuple = await FILE_SAVER.getTuple(putResultConfig);

  it('getTuple -> tuple.config -> should be equal to putResultConfig', async () => {
